...
public class ContainerList {
    ...
    private void initializePattern(@NotNull AbstractPattern pattern) {
        pattern.setCenter(new Coordinate(pattern.getTotalPatternLength() / 2, pattern.getTotalPatternWidth() / 2, pattern.getTotalPatternHeight() / 2));
        this.patterns.add(pattern);
        this.containerVolumeLeft -= pattern.getPatternVolume();
        this.containerLengthLeft -= pattern.getTotalPatternLength();
    }
    ...
    public boolean isPatternFitInContainer(@NotNull AbstractPattern pattern) {
        return containerInfo.getHeight() > pattern.getTotalPatternHeight() && containerInfo.getWidth() > pattern.getTotalPatternWidth();
    }
