...
const CanvasSection: React.FC<CanvasSectionProps> = ({ boxes, pallets, reducedDimensions, cameraPosition, zoom, farFactor }) => (
    <div style={{ flexDirection:'row', width: '100%', height: '25%' }}>
        <div style={{width:'50%'}}>
            <label style={{textAlign: 'center', fontSize: '20px'}}>Container</label>
        </div>
        <Canvas orthographic camera={{ zoom: zoom, position: cameraPosition, near: 0.1, far: farFactor }} style={{ width: '50%', height: '100%' }}>
            <ambientLight />
            ...
            {pallets.map((pallet, index) => (
                <group key={index} position={new THREE.Vector3(pallet.x - 150, pallet.y - 115, pallet.z - 650)}>
                    {boxes.map((box, boxIndex) => (
                        <BoxDraw key={boxIndex} center={[box.x, box.y, box.z]} dimensions={[box.width, box.height, box.length]} rotate={box.rotate} orderIndex={box.orderIndex} color={box.color}/>
                    ))}
                </group>
            ))}
        </Canvas>
